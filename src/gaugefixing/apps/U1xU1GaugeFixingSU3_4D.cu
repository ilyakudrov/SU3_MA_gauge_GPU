...
int main(int argc, char* argv[])
{
	...
	U1xU1KernelsSU3::initCacheConfig();
	...
	GaugeFixingStats<Ndim,Nc,U1xU1KernelsSU3,AVERAGE> gaugeStats( dU, HOST_CONSTANTS::SIZE );
	...
	for( fi.reset(); fi.hasNext(); fi.next() )
	{
		...
		// output.precision(17);
		// output.open("SA_test");
		// output << "temperature,functional"<<endl;
		...
		for( int copy = 0; copy < options.getGaugeCopies(); copy++ )
		{
			...
			{
				U1xU1KernelsSU3::randomTrafo(numBlocks,threadsPerBlock,dU, dNn, 0, options.getSeed(), PhiloxWrapper::getNextCounter() );
				U1xU1KernelsSU3::randomTrafo(numBlocks,threadsPerBlock,dU, dNn, 1, options.getSeed(), PhiloxWrapper::getNextCounter() );
			}
			...
			do
			{
				...
				if((temperature <= 2.7) && (temperature >= 2))
				...
				U1xU1KernelsSU3::saStep(numBlocks,threadsPerBlock,dU, dNn, 0, temperature, options.getSeed(), PhiloxWrapper::getNextCounter() );
				U1xU1KernelsSU3::saStep(numBlocks,threadsPerBlock,dU, dNn, 1, temperature, options.getSeed(), PhiloxWrapper::getNextCounter() );

				for( int mic = 0; mic < options.getSaMicroupdates(); mic++ )
				{
					U1xU1KernelsSU3::microStep(numBlocks,threadsPerBlock,dU, dNn, 0 );
					U1xU1KernelsSU3::microStep(numBlocks,threadsPerBlock,dU, dNn, 1 );
				}
			...
			{
				U1xU1KernelsSU3::orStep(numBlocks,threadsPerBlock,dU, dNn, 0, options.getOrParameter() );
				U1xU1KernelsSU3::orStep(numBlocks,threadsPerBlock,dU, dNn, 1, options.getOrParameter() );
